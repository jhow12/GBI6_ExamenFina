...
write.table(x = dt, file = "results/mRNA_expr_summary.doc", sep = ",", 
...
tcga_boxplots <- function(data){
  cd <- df_long %>% filter (dataset == data)
  dc <- cd %>% ggplot(aes(x=gen,y=expresion_level, color = gen)) +
...
tipo_decancer <- c("BRCA","OV","LUSC","KIPAN","KIRP","UCEC")
cancertype_boxplots <- lapply(tipo_decancer, tcga_boxplots)
cancertype_boxplots
